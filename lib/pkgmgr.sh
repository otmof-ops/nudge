...
flatpak_upgrade() {
    if flatpak_available && [[ "$PKG_UPDATES_FLATPAK" -gt 0 ]]; then
        ...
        if ! flatpak update -y 2>/dev/null; then
            log_warn "Flatpak upgrade failed (may require interactive auth)"
        fi
    fi
}
...
snap_upgrade() {
    if snap_available && [[ "$PKG_UPDATES_SNAP" -gt 0 ]]; then
        ...
        if ! sudo snap refresh 2>/dev/null; then
            log_warn "Snap refresh failed (may require interactive auth)"
        fi
    fi
}
...
pkgmgr_build_json_packages() {
    ...
    if [[ -n "$PKG_UPDATE_LIST" ]]; then
        while IFS='|' read -r name from_ver to_ver priority; do
            ...
            json+="{\"name\":\"$(json_escape "$name")\",\"from\":\"$(json_escape "$from_ver")\",\"to\":\"$(json_escape "$to_ver")\",\"priority\":\"$(json_escape "$priority")\"}"
