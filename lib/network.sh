...
network_probe_once() {
    ...
    if command -v curl &>/dev/null; then
        if curl --head --silent --max-time "$timeout" --output /dev/null "http://$host" 2>/dev/null; then
    ...
    if command -v wget &>/dev/null; then
        if wget --spider --timeout="$timeout" --quiet "http://$host" 2>/dev/null; then
        ...
        notify)
            ...
            if command -v notify-send &>/dev/null; then
                notify-send -i network-offline "nudge" \
                    "Network unavailable, skipping update check" 2>/dev/null || true
            fi
            return "$EXIT_NETWORK_FAIL"
            ;;
        queue)
