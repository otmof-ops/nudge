...
- **21 new config keys** (32 total): CONF_VERSION, SCHEDULE_MODE, SCHEDULE_INTERVAL_HOURS, HISTORY_ENABLED, HISTORY_MAX_LINES, FLATPAK_ENABLED, SNAP_ENABLED, PREVIEW_UPDATES, SECURITY_PRIORITY, REBOOT_CHECK, SNAPSHOT_ENABLED, SNAPSHOT_TOOL, SELF_UPDATE_CHECK, SELF_UPDATE_CHANNEL, OFFLINE_MODE, DEFERRAL_OPTIONS, PKGMGR_OVERRIDE, DUNST_APPNAME, EXIT_ON_HELD, JSON_OUTPUT, LOG_LEVEL
...
### Changed
- `nudge.sh` rewritten as ~150-line thin dispatcher sourcing 10 modules from `lib/`
- `install.sh` rewritten with full wizard for all 32 config keys, systemd/XDG choice, upgrade support
- `uninstall.sh` upgraded to clean up systemd units, bash completion, man page, history, state files
- `nudge.conf` expanded from 11 to 32 fully documented configuration keys
