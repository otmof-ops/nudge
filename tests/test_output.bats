...
@test "json_escape handles newline and tab" {
    local input=$'line1\nline2\ttab'
    local result
...
@test "json_escape handles carriage return" {
...
@test "json_escape handles control characters" {
    local input=$'\x01\x02'
    local result
    result=$(json_escape "$input")
    [[ "$result" == '\u0001\u0002' ]]
...
@test "json_escape does not double-escape" {
    local result
    result=$(json_escape 'already\\escaped')
    [[ "$result" == 'already\\\\escaped' ]]
}
